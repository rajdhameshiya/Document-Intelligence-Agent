...
export function getSampleText(fileName = ''): string {
  const normalized = fileName.toLowerCase();
  if (normalized.includes('booking') || normalized.includes('bc')) return SAMPLE_BOOKING_CONFIRMATION_TEXT;
  if (normalized.includes('invoice') || normalized.includes('ci')) return SAMPLE_CI_TEXT;
  if (normalized.includes('packing') || normalized.includes('pl')) return SAMPLE_PL_TEXT;
  return SAMPLE_SI_TEXT;
}
